...
var $gnavItem = document.querySelectorAll(".gnav__item");
/*--アンカーリンク処理 --- */
...
var readyInit = function readyInit() {
  ...
  var _loop2 = function _loop2(ancCount) {
    var ancObj = document.getElementById($ancLink[ancCount].getAttribute("href").slice(1));
    ancTopArr[ancCount] = _sTop + ancObj.getBoundingClientRect().top;
    $ancLink[ancCount].addEventListener("click", function (e) {
      e.preventDefault();
...
var stgSLInAnime = function stgSLInAnime($obj) {
  ...
  if ($obj.classList.contains('js-stgSL-left')) {
    TweenMax.staggerFromTo($obj.children, .3, {
      x: -1 * stgSLIval,
      opacity: 0
    }, {
      x: 0,
...
var scrollAnc = function scrollAnc($object, $speed, $scroll) {
  var hash = $object;
  var $target;
  var t_hash; // let h_h = document.querySelector(".header").clientHeight;
  // console.log($scroll);
...
if (!_SP) {
  var _loop3 = function _loop3(hoverCount) {
    $hover[hoverCount].addEventListener("mouseover", function (e) {
      TweenMax.to($hover[hoverCount], .2, {
        opacity: .5
      });
    });
    $hover[hoverCount].addEventListener("mouseout", function (e) {
      TweenMax.to($hover[hoverCount], .2, {
        opacity: 1,
        onComplete: function onComplete() {
